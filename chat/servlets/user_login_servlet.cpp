...
int32_t UserLoginServlet::handle(sylar::http::HttpRequest::ptr request
    ...
    do {
        ...
        chat::data::UserInfo::ptr info;
        // 验证账号和邮箱是否合法
        // 验证完后，获取在create后端存储过的账号/邮箱
        if(is_valid_account(auth_id)) {
            info = chat::UserMgr::GetInstance()->getByAccount(auth_id);
        }
        else if(is_email(auth_id)) {
            info = chat::UserMgr::GetInstance()->getByEmail(auth_id);
        }
        else {
        ...
        if(chat::data::UserInfoDao::Update(info, db)) {
        ...
        std::string token = UserMgr::GetInstance()->GetToken(info, token_time);
