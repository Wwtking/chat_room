...
bool UserManager::loadAll() {
    ...
    std::vector<chat::data::UserInfo::ptr> results;
    if(chat::data::UserInfoDao::QueryAll(results, db)) {
        SYLAR_LOG_ERROR(g_logger) << "UserManager loadAll fail";
        return false;
    }

    std::unordered_map<int64_t, chat::data::UserInfo::ptr> datas; 
    std::unordered_map<std::string, chat::data::UserInfo::ptr> accounts;
    std::unordered_map<std::string, chat::data::UserInfo::ptr> emails;
    std::unordered_map<std::string, chat::data::UserInfo::ptr> names;
...
// 添加用户数据
void UserManager::add(chat::data::UserInfo::ptr info) {
    MutexType::WriteLock lock(m_mutex);
...
// 根据id获取用户数据
chat::data::UserInfo::ptr UserManager::get(int64_t id) {
    XX(m_datas, id);
}

// 根据account获取用户数据
chat::data::UserInfo::ptr UserManager::getByAccount(std::string v) {
    XX(m_accounts, v);
}

// 根据email获取用户数据
chat::data::UserInfo::ptr UserManager::getByEmail(std::string v) {
    XX(m_emails, v);
}

// 根据name获取用户数据
chat::data::UserInfo::ptr UserManager::getByName(std::string v) {
    XX(m_names, v);
}
